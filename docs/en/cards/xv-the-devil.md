---
title: XV - The Devil
description: "Devil Room While held, fatal damage revives you into a Devil Room Using it actively spends your life to trigger the same effect"
slug: xv-the-devil
kind: card
internalKey: Devil
status: drafted
---
<p class="wiki-search-index" v-pre>XV - 邪心 XV - The Devil Devil xv-the-devil XV - The Devil 灵魂算子：重生 恶魔房 持有时可在致命伤害时复活，并前往恶魔房 主动使用则立即献出生命并触发该效果 Devil Room While held, fatal damage revives you into a Devil Room Using it actively spends your life to trigger the same effect</p>

<PublicEntry slug="xv-the-devil" lang="en" />

## Mechanics

<!-- Manual body. The exporter never overwrites this file once it exists. -->

## Effects

While held, fatal damage revives into a Devil Room; active use spends life to trigger the same. The held Devil card is consumed on that revival.
