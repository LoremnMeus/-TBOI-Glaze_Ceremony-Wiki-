...
在持有蓝图、且本房曾有可清理敌人的普通清房中，原型基础出现概率约为：
...
- 高价值武器模块的原型，优先留给已经有不错底座、但缺关键攻击方式的机体。
- 交易房原型保留池主题：不要假设恶魔房会出现任意宝箱房模块。
- 清房保底在“一直不出”时值得记一笔；有蓝图却长期看不到原型，往往是还没走到保底段。
