---
title: 恶魔的心智
description: "他们自愿为我而死"
slug: devils-heart
kind: collectible
internalKey: Devil_s_Heart
status: drafted
---
<p class="wiki-search-index" v-pre>恶魔的心智 Devil&#x27;s Heart Devil_s_Heart devils-heart Devil&#x27;s Heart 他们自愿为我而死 Stage a Comeback 向敌人植入恶魔种子，与其缔结替死契约 你受到致死伤害时，宿主会替你承担死亡 宿主先受到致死伤害时，你会替它承担代价 Plant a devil seed to bind a two-way death pact If you take lethal damage, the host dies in your place If the host would die first, you pay the cost instead</p>

<PublicEntry slug="devils-heart" lang="zh" />

## 机制说明

<!-- 人工正文：生成器不会覆盖本文件。把玩法、联动、Neta、版本历史写在这里。 -->

## 效果

**恶魔的心智与一名敌人缔结双向替死契约：谁先该死，另一方就来付账。**

使用后举起，再按射击键朝该方向掷出种子。种子命中可受伤敌人后，对方成为宿主。

| 谁先致死 | 结果 |
| --- | --- |
| 角色受到致死伤害 | 取消角色死亡；传送到宿主位置，由宿主代付。普通敌直接死亡；Boss 扣除约 **12%** 最大生命且不会被这一击直接杀死 |
| 非 Boss 宿主受到致死伤害 | 取消宿主死亡，将其生命拉回约 **20%** 最大生命；角色受到 **1** 颗整心伤害 |
| Boss 宿主受到致死伤害 | 契约解除，不触发角色代付 |

每次宿主成功替死，角色会叠加一层相关外观（最多四层），用于标记契约已兑现的次数。

## 特殊联动

### {{Collectible:584}}

消耗一枚魂火时：普通宿主改为重伤保留，Boss 代付伤害减半。

### {{Collectible:34}}

宿主替死后，本房间获得临时攻击提升。

### {{Seija}}

角色替宿主付钱时只受半心伤害；宿主替角色付钱时，角色仍会额外受到一点伤害。

### {{Collectible:706}}

对应蝗虫命中敌人时也可以植入契约。
