---
title: 最终棱镜
description: "异世界的赠礼"
slug: final-prism
kind: collectible
internalKey: Final_Prism
status: reviewed
---
<p class="wiki-search-index" v-pre>最终棱镜 Final Prism Final_Prism final-prism Final Prism 异世界的赠礼 A gift from another world 自动恢复充能 使用后维持6道彩色激光，并持续消耗充能 持续照射时，光束会逐渐聚拢并大幅增强 再次使用可额外消耗充能，增加光束数量 Recharges automatically Maintains 6 rainbow lasers while draining charge Beams gradually converge and grow much stronger Use again to spend extra charge and add more beams</p>

<PublicEntry slug="final-prism" lang="zh" />


## 机制说明

<!-- 人工正文：生成器不会覆盖本文件。把玩法、联动、Neta、版本历史写在这里。 -->

## 效果

**最终棱镜是维持型蓄力武器：持续照得越久越强，而不是按一下就结束的激光。**

首次使用启动约 **6** 束彩色激光。开启后改为边照边耗充能：充能为 0 时激光停止。满充大约只能维持十余秒，需要边打边等自动回充。

照射时间越长，光束越聚拢、表现与伤害越高；大约 **40** 秒持续照射才能爬到完整形态。

已开启时再按主动，会额外花费约 **10** 点充能，并把光束数量再 **+3**。

玩家应把它理解成：**先开火预热，再决定是否砸充能加束。**

## 使用技巧

- 开打后尽量让棱镜持续对准目标，预热收益比频繁开关更重要。
- 充能见底前可以停一下回充，再打第二轮预热。
- 只有在需要更高密度火力时才二次投入加束。

## 特殊联动

### {{Collectible:34}}
光束改为彩色硫磺火表现。

### {{Collectible:584}}
生成与角色一同发射激光的魂火。

### {{Collectible:356}}
影响再次使用时的追加光束结算。
