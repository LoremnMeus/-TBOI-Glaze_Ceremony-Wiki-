---
title: 宝宝玛丽
description: "我来质疑！"
slug: baby-marri
kind: collectible
internalKey: Baby_Marri
status: reviewed
---
<p class="wiki-search-index" v-pre>宝宝玛丽 Baby Marri Baby_Marri baby-marri Baby Marri 我来质疑！ I question! 初始为天使形态：每只宝宝使天使房转化率+15% 受到伤害后切换为恶魔形态：每只使天魔房开启率+15% 再次受到伤害时切回天使形态 Starts in Angel form: +15% Angel Room conversion per baby Taking damage swaps to Devil form: +15% Devil/Angel deal chance per baby Taking damage again swaps back to Angel form</p>

<PublicEntry slug="baby-marri" lang="zh" />


## 机制说明

<!-- 人工正文：生成器不会覆盖本文件。把玩法、联动、Neta、版本历史写在这里。 -->

## 效果

**宝宝玛丽在天使与恶魔两种形态间切换，分别强化两条不同的天魔路线。**

- 初始为 **天使形态**：每只宝宝使「天使房转化率」**+15%**
- 受到伤害后切换为 **恶魔形态**：每只宝宝使天魔房开启率 **+15%**
- 再次受伤切回天使；形态会持久保存

## 「天使转化率」是什么

它加的是：**天魔交易已经成功开房之后，把房间转成天使房的概率**。
不是额外凭空多开 15% 天使房。

恶魔形态则直接提高天魔房总开启率（先于转化率结算）。

## 如何触发

形态切换由受伤自动触发；对应加成在当前形态下持续生效。

## 重复持有

每只宝宝各自贡献 +15%；多只宝宝共享同一玩家的形态状态。

## 轶事

模仿对应角色在天使/恶魔抉择上的气质，不是完整角色机制。
