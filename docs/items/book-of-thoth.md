...
透特之书会记录玩家在本局中见过的透特牌。收集不同牌面获得启示，再消耗启示从已记录的牌中组成牌阵，使它们在之后的战斗房中自动发动。
...
持有透特之书期间，第一次把某种透特牌放入卡牌栏时，对应牌面会被收录进卡册。正位和逆位分别记录；收录不会消耗卡牌，之后使用或丢弃它也不会抹去记录。
...
牌阵建立后，每次进入一个新的未清理战斗房，都会从尚未发动的牌中随机选择一张并免费使用。所有牌都发动后，本次牌阵结束，玩家才能建立下一组牌阵。
...
> 本页只使用模组自身素材和游戏截图，不收录现代出版物中的 Thoth Tarot 卡面扫描。
