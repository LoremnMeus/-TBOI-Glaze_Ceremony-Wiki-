...
琉璃的冠冕靠不断拾取琉璃掉落加冕，再在受伤碎冠的风险下维持高层效果。

拾取任意受支持的琉璃掉落 → **+1** 辉片，最多 **5** 层。层数越高，冠冕逐步解锁攻击与琉璃相关能力。真正受到有效伤害时，全部辉片一次性破碎，并按失去的层数向四周释放碎片。
...
碎冠本身也是一次反击：按原层数向外射出碎片，并有机会额外掉落一份随机琉璃资源（同一次碎冠最多一份）。碎掉不是纯损失。

## 注意

- 3 层的触发率加成只作用于冠冕自身的折射与敌人琉璃化，不会放大所有琉璃系掉落的效果概率。
- 圣盾实际挡住的伤害不会碎冠。
...
琉璃掉落生成提升被压低；碎冠伤害减半；3 层不再提供那项触发率加成。

## 使用技巧

- 接近满层时，先想清楚下一份琉璃掉落值不值得冒碎冠风险去保。
- 碎冠后仍可能吐回琉璃资源，受伤不全是白亏——但高层碎冠仍会失去整套层效果。

<details>
<summary>技术细节</summary>

- 2 层折射基础约 22%；每次折射生成两发侧向碎泪，各约 `30%` 玩家攻击。
- 3 层起，折射 / 敌人琉璃化的基础触发系数约 `1.25×`（Seija 下为 `1×`）。
