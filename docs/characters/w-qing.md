...
{{Character:w-qing}} 使用小刀取代普通眼泪。近距离可以直接挥砍；投出的刀可插入敌人、拾取物及部分有效对象。使用返回操作后，小青会迅速前往选中的插刀位置，并回收对应的小刀。
...
存在多个可用坐标时，它们会分别保留，不会合并成同一个落点。

### 防困保护

缺少安全坐标时，系统会保留必要的小刀作为脱困路径。该保护不会让普通投刀无视房间地形。
