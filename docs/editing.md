...
2. 正文是否出现代码、仓库、脚本、callback、当前实现、设计上、不是漏洞、这是成本等开发者或设计辩护视角？
3. 是否为了显得完整，把不影响首次理解或玩家决策的低价值边界规则放进主正文？
4. 简单内容是否被强行拆成五六个章节，而不是只保留真正有独立查询价值的结构？
5. 复杂内容的精确规则是在回答玩家会问的问题，还是只在复述实现？

第 2 项只要命中，原则上不得提升为 `reviewed`：先改写为玩家可观察的结果，或从公开正文删除。其他任一项不通过时也应继续保持 `drafted`，完成编辑返工后再复审。Editorial Pass 不能替代 Lua/XML 事实核验；两项都通过，才可标记 `reviewed`。
