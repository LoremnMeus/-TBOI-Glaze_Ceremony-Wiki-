---
title: "XVII - 星辰?"
description: "他们灿若繁星"
slug: xvii-the-stars-reversed
kind: card
internalKey: Star_r
status: reviewed
---
<p class="wiki-search-index" v-pre>XVII - 星辰? XVII - The Stars? Star_r xvii-the-stars-reversed XVII - The Stars? 他们灿若繁星 伯列恒之星 点亮房间内的敌人并为其添加光环 伯列恒之星 在此卡的周围自动提供一个有50%增幅效果的光圈 Star of Bethlehem Light enemies in the room and give them an aura Star of Bethlehem Also provides a 50% boost aura around this card</p>

<PublicEntry slug="xvii-the-stars-reversed" lang="zh" />

## 机制说明

<!-- 人工正文：生成器不会覆盖本文件。把玩法、联动、Neta、版本历史写在这里。 -->


## 效果

使用后，当前房间的敌人会被点亮，并在周围形成光环。靠近这些星光源，或靠近仍在地上的这张牌时，玩家会获得类似 {{Collectible:651}} 的强化：{{Damage}} 与 {{Tears}} 提升；当强化强度超过一半时，还会附带幽灵泪，且受到伤害有 **50%** 概率完全抵挡这次伤害。

## 规则

强化强度取决于与星光源的距离。

## 特殊联动

### {{Collectible:451}}

本层内之后新出现的敌人也会被点亮。当前房间内已有的掉落物会立即成为星光源，之后新出现的掉落物同样会成为星光源。该效果持续到换层。
